"""Fast path when each municipality is a single municipal aggregate.
...
When the dataset is all municipal aggregates, ``ensure_muni_agg_ram_cache`` loads every
...
_RAM_CACHE_ATTR = "_muni_agg_ram_cache"
_RAM_CACHE_LOGGED = False


def is_muni_agg_batch(num_pixels_list) -> bool:
    if not num_pixels_list:
        return False
    return max(int(n) for n in num_pixels_list) <= 1


def dataset_is_muni_agg(dataset) -> bool:
...
def dataset_supports_muni_agg_stack(dataset) -> bool:
...
def _log_fastpath_once(n: int) -> None:
    ...
    print(
        f"Municipal-aggregate batch: stacking {n} municipalities into one forward "
...
def ensure_muni_agg_ram_cache(dataset, *, workers: int = 8) -> dict | None:
    """
    Preload every municipal aggregate series into ``dataset._muni_agg_ram_cache``.
    ...
    Returns the cache dict, or None when the dataset is not all municipal aggregates.
    """
    global _RAM_CACHE_LOGGED
    existing = getattr(dataset, _RAM_CACHE_ATTR, None)
    if isinstance(existing, dict):
        return existing
    if not dataset_supports_muni_agg_stack(dataset) or not dataset_is_muni_agg(dataset):
        return None
    ...
    if not _RAM_CACHE_LOGGED:
        ...
        print(
            f"Municipal-aggregate RAM cache: {len(cache)} series ({kind}, "
...
def load_stacked_muni_agg_batch(
    dataset,
    municipalities,
    years,
    *,
    ...
    when nothing loaded.
    """
    ensure_muni_agg_ram_cache(dataset, workers=workers)
...
def forward_muni_agg_batch(
    ...
    x = muni_agg_on_device(stacked, device, args)
    mark_cudagraph_step()
...
def process_train_muni_agg_batch(
    *,
    model,
    optimizer,
    scaler,
    ...
    if stacked is None or not kept:
        ...
        if vram_probe_state is None:
            print(
                f"  ⚠️  DEBUG: Skipping optimizer step for batch {batch_idx} - no muni_aggs loaded"
            )
        return 0.0, False, 0
