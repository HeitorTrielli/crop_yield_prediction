...
MoCo Paraná trunk. Shares ``PositionalEncoding`` / ``linlayer`` with ``STNet`` /
``STNetRegression`` so checkpoint weight names stay consistent for transfer.
"""
...
class TransformerModel(nn.Module):
    ...
    def forward(self, x, use_doy=False, is_bert=False):
        ...
        if use_doy:
            ...
            src_pos = (
                torch.arange(1, self.max_seq_len + 1, dtype=torch.long)
                .expand(b, s)
                .cuda()
            )
